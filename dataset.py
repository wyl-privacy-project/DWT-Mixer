    ...
    def setup(self, stage: str = None):
        ...
        if stage in (None, 'fit'):
            self.train_set = dataset_cls( 'train', self.train_cfg.max_seq_len, self.tokenizer, self.projecion,
                                         self.label_map,self.bert_tokenizer,self.em_mode)
...
class TSMixerDataset(Dataset):
    def __init__(self, max_seq_len: int, tokenizer: Tokenizer, projection: Projection, label_map: Dict[str, int],berttoken,emmode,
                 **kwargs):
    ...
    def project_features(self, words: List[str]):
        encoded = self.bert_tokenizer.encode_plus(words, is_pretokenized=True, add_special_tokens=False,max_length=self.max_seq_len,
        padding='max_length', return_tensors='pt',)
        # print(encoded)
        # tokens = [[] for _ in range(len(words))]
        # for index, token in zip( encoded,self.bert_tokenizer.convert_ids_to_tokens(encoded)):
        #     tokens[index].append(token)
        # features = self.projection(self.bert_tokenizer.convert_ids_to_tokens(encoded))
        # padded_featrues = np.pad(features, ((0, self.max_seq_len - len(words)), (0, 0)))
        return encoded['input_ids'],encoded['token_type_ids']

    def project_features2(self, words: List[str]) -> np.ndarray:
    ...
    def __getitem__(self, index) -> Dict[str, Any]:
        ...
        if self.em_mode=="bert":
            features,token_id = self.project_features(words)
            ...
            features = self.project_features2(words)
            labels = self.compute_labels(fields)
            return {
                'inputs': features,
                'targets': labels,
...
class QQPDataset(TSMixerDataset):
    ...
    def __getitem__(self, index) -> Dict[str, Any]:
        fields = self.data[index]
        words = self.get_words(fields)
        u = self.project_features2(words[0]).reshape(1, self.max_seq_len, -1)
        v = self.project_features2(words[1]).reshape(1, self.max_seq_len, -1)
        features = np.concatenate((u, v), axis=0)
        labels = self.compute_labels(fields)
        return {
            'inputs': features,
